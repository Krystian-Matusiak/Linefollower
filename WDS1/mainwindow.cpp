...
void MainWindow::GdyNapis(const QString &Napis)
{
...
void MainWindow::GdySekunda(){
  ...
  /*ui->PWM_lewo_val->setText(QString::number(*DaneSensoryczne->pwm1));
  if(DaneSensoryczne->dir_motor1)   ui->Kier_lewo_val->setText("Przod");
  else  ui->Kier_lewo_val->setText("Tył");

  ui->PWM_prawo_val->setText(QString::number(*DaneSensoryczne->pwm2));
  if(DaneSensoryczne->dir_motor2)   ui->Kier_prawo_val->setText("Przod");
  else  ui->Kier_prawo_val->setText("Tył");

  // Wypisywanie w tabeli danych czujnikowych
  ui->Czuj1_val->setText( QString::number( DaneSensoryczne->CzujnikiOdb[0] ) );
  ui->Czuj2_val->setText( QString::number( DaneSensoryczne->CzujnikiOdb[1] ) );
  ui->Czuj3_val->setText( QString::number( DaneSensoryczne->CzujnikiOdb[2] ) );
  ui->Czuj4_val->setText( QString::number( DaneSensoryczne->CzujnikiOdb[3] ) );
  ui->Czuj5_val->setText( QString::number( DaneSensoryczne->CzujnikiOdb[4] ) );
  ui->Czuj6_val->setText( QString::number( DaneSensoryczne->CzujnikiOdb[5] ) );

  // Uaktualnienie danych związanych z wykresem
  DaneSensoryczne->newPlotY(DaneSensoryczne->pwm1);
  DaneSensoryczne->newPlotZ(DaneSensoryczne->pwm2);


  makeGraph( DaneSensoryczne->CzujnikiOdb[0],
             DaneSensoryczne->CzujnikiOdb[1],
             DaneSensoryczne->CzujnikiOdb[2],
             DaneSensoryczne->CzujnikiOdb[3],
             DaneSensoryczne->CzujnikiOdb[4],
             DaneSensoryczne->CzujnikiOdb[5]
          );
  makePlot();
  setArrows( *DaneSensoryczne->pwm1 , *DaneSensoryczne->pwm2 );*/
  setBattery(*DaneSensoryczne->pwm1);
...
MainWindow::MainWindow(DanePomiarowe *DP, QWidget *parent)
    : QMainWindow(parent)
    , ui(new Ui::MainWindow)
{
    ...
    _Wiadomosc = DP->info1[0];
    _Wiadomosc2 = DP->info1[1];
    _Wiadomosc3 = DP->info1[2];
    _Wiadomosc4 = DP->info1[3];
...
void MainWindow::Czuj_trans(int c1, int c2, int c3, int c4, int c5, int c6){
...
void MainWindow::Silniki(int dir1, int pwm1, int dir2, int pwm2){
...
void MainWindow::makePlot(){
    ...
    {
      x[i] = end*i/sample ;
      y[(int)i] = DaneSensoryczne->plotY[(int)i] ;
      z[(int)i] = DaneSensoryczne->plotZ[(int)i] ;
    }
